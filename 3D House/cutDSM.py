"""This .py file cuts the downloaded Digital Surface Model raster files in tiles of 1000m by 1000m
...
def writeImageAsGeoTIFF(img : np.ndarray, transform : np.ndarray, metadata : Dict[str,Union[str,int,np.ndarray]],
    ...
    metadata.update({"driver":"GTiff",
                     ...
                     "transform": transform,
                     "crs": crs})
    # FOR DSM : write tile to file
    with rio.open('/media/becode/EXT/DSMsplit/' +filename+'.tif', "w", **metadata) as dest:
        dest.write(img)

def main():
    for i in range(1,44): # loop to read in all 43 downloaded DSM raster files
        if i < 10 :
            ...
            path_in = "/media/becode/EXT/DSM/DHMVIIDSMRAS1m_k" + str(i) + "/GeoTIFF/DHMVIIDSMRAS1m_k" + str(
                i) + ".tif"
        plot_dsm = rio.open(path_in) # open raster file to read in
        splitImageIntoCells(plot_dsm, "tile_dsm_k" + str(i), 1000) #split specified raster file in 1000x1000 tiles
